...
export function NewInvoiceDialog({ open, onClose, onCreated, customerId: defaultCustomerId, vehicleId: defaultVehicleId }: Props) {
  ...
  useEffect(() => {
    if (customerId) {
      fetch(`/api/vehicles?customerId=${customerId}`).then(r => r.json()).then(setVehicles);
    } else {
      setVehicles([]);
    }
  }, [customerId]);
        ...
        <div className="space-y-5 mt-2">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <Select value={vehicleId} onValueChange={setVehicleId} disabled={!vehicles.length}>
              <SelectTrigger label="Vehicle (optional)">
          ...
          <div>
            ...
            <div className="space-y-2">
              {items.map((item, idx) => (
                <div key={idx} className="grid grid-cols-12 gap-2 items-center">
                  ...
                  <div className="col-span-4">
                    <input
                      className="w-full h-10 px-3 border border-[#e5e5e3] bg-white text-sm focus:outline-none focus:border-[#0B0B0C]"
                  ...
                  <div className="col-span-1">
                    <input
                      className="w-full h-10 px-2 border border-[#e5e5e3] bg-white text-sm focus:outline-none focus:border-[#0B0B0C] text-center"
                      type="number"
                      min="1"
                      value={item.quantity}
                  ...
                  <div className="col-span-2">
                    <input
                      className="w-full h-10 px-2 border border-[#e5e5e3] bg-white text-sm focus:outline-none focus:border-[#0B0B0C]"
                      type="number"
                      min="0"
                      step="0.01"
                      placeholder="Price"
                  ...
                  <div className="col-span-1 flex items-center justify-end">
                    {items.length > 1 && (
                      <button onClick={() => removeItem(idx)} className="text-[#707070] hover:text-red-500">
            ...
            <button onClick={addItem} className="mt-2 flex items-center gap-1 text-sm text-[#C9A86A] hover:underline">
              <Plus className="h-4 w-4" /> Add Line Item
            </button>
          </div>

          {/* Totals */}
          <div className="border-t border-[#e5e5e3] pt-4 space-y-2">
            <div className="grid grid-cols-2 gap-3">
            ...
            <div className="flex items-center justify-between text-sm font-semibold text-[#111111] pt-2">
              <span>Total</span>
